...
describe('bitgetAdapter', () => {
  it('should have the correct name', () => {
    expect(bitgetAdapter.name).toBe('Bitget');
  });

  it('should fetch real P2P orders from Bitget API (integration)', async () => {
    const orders = await bitgetAdapter.fetchP2POrders('USDT', 'USD', 'BUY');
    expect(Array.isArray(orders)).toBe(true);
    expect(orders.length).toBeGreaterThan(0);
    expect(orders[0]).toHaveProperty("id");
    expect(orders[0]).toHaveProperty("exchange", "Bitget");
